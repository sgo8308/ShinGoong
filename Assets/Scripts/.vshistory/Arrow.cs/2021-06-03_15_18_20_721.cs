...
public class Arrow : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (GetComponent<Rigidbody2D>().gravityScale != 0 && GetComponent<Rigidbody2D>().bodyType != RigidbodyType2D.Kinematic) //곡사가 충돌할때 화살이 박힌다.
        {
            Debug.Log("곡사 충돌 시작!");
    ...
    void DetachedFromMonster()
    {
        ...
        transform.Find("arrowCollider").gameObject.SetActive(true);
    }
}
