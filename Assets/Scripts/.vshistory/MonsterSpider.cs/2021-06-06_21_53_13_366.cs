...
public class MonsterSpider : Monster
{
    ...
    void FixedUpdate()
    {
        ...
        //Platform Check floor
        Vector2 frontVecVertical = new Vector2(_rigid.position.x + _nextMove, _rigid.position.y);
        Debug.DrawRay(frontVecVertical, Vector3.down, new Color(0, 1, 0), 10.0f, false);
        ...
        //Platform Check Wall
        Vector2 frontVecHorizontal = new Vector2(_rigid.position.x + _nextMove, _rigid.position.y);
        Debug.DrawRay(frontVecHorizontal, Vector3.left, new Color(0, 1, 0), rayLength, false);

        RaycastHit2D rayHit2 = Physics2D.Raycast(frontVecHorizontal,
                                Vector3.left, rayLength, LayerMask.GetMask("Platform"));
